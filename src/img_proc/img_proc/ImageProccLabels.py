...
class OakCameraNode(Node):
    def __init__(self):
        ...
        self.nnPath = str(
            Path.home() / "kasif_celebi_ws/models/yolov8n_coco_640x352.blob"
        )
        ...
        # Sadece bu sınıflar görüntüye çizilecek
        self.allowed_labels = [
            0,   # person
            14,  # bird
            15,  # cat
        ...
        self.get_logger().info("OAK person/object detector node started")
    ...
    def frameNorm(self, frame, bbox):
        ...
        return (
            np.clip(np.array(bbox), 0, 1) * normVals
        ).astype(int)
    ...
    def draw_detections(self, frame):
        ...
        for detection in self.detections:
            ...
            # İzin verilen sınıflar dışındakileri çizme
            if label_id not in self.allowed_labels:
                continue

            if label_id < len(self.labelMap):
        ...
        fps = self.counter / max(
            (time.monotonic() - self.startTime),
            1e-6
        )
